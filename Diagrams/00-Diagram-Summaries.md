...
Entity identifiers use UUID primary keys (`Ticket.id` included, via Prisma's `uuid()` default) — with one category of exception, not zero: the many-to-many bridge tables (`TicketAsset` here, `TicketKnowledgeArticle` in diagram 3) have no surrogate `id` at all and instead use a **composite primary key made of their two UUID foreign keys** (`@@id([ticketId, assetId])`). `Ticket.ticketNumber` is a separate concern again — a real database-sequence auto-incrementing integer that exists purely so a person can say "ticket #4821" out loud; it was never a primary-key candidate at all, so the precise description is "UUID primary keys for entities, composite UUID-FK primary keys for bridge tables, plus a separate unique human-readable `ticketNumber` on `Ticket`." Several fields carry a `deletedAt` timestamp instead of being hard-deleted (`User`, `Ticket`, `TicketComment`, `Asset`) so a "removed" record still makes sense in historical reports and audit trails; `TicketHistory`, `TicketCategory`, `SlaPolicy`, `TicketSla`, `AssetType`, `AssetAssignment`, and `TicketAsset` are hard-delete-only (no `deletedAt` column exists for them).
...
- **AI Assistant** — optional, staff-only, advisory: it can draft a triage suggestion, a reply, or a resolution summary, but nothing it produces changes a ticket by itself — a suggestion is applied only by a second, explicit ticket-mutation call (see diagram 11) that gets the same validation, concurrency check and history write as a change a person typed.
...
### The cross-tab race — real, but the guarantee is conditional
"Two tabs racing a refresh is handled by client-side locking" is not a hypothetical in the code: `frontend/src/lib/api/client.ts` genuinely uses `navigator.locks` (Web Locks API), with its own dedicated tests, to serialize refresh attempts. But the strength of that guarantee depends on browser support, and the code's own comment says so directly: **where `navigator.locks` is available, it serializes refresh attempts across same-origin tabs**, so a legitimate second tab waits and presents the winner's freshly-rotated cookie rather than a stale one. Where it's unavailable (`jsdom` in tests, or any non-secure context) the code falls back to a plain in-tab single-flight promise (`refreshInFlight`) — that fallback **only deduplicates concurrent refresh calls within the current tab's JS instance**; it provides no coordination across tabs at all. So "a second tab never triggers the reuse/theft path" is true when Web Locks is available, and not a guarantee the fallback path provides.
...
### Key takeaway
The CI pipeline independently repeats the application's local quality checks (typecheck, lint, unit/e2e tests, build) against a real, ephemeral environment rather than a mock one, and adds container/image validation on top that has never been possible to run locally on this development machine, since Docker itself isn't installed here (see diagram 10) — the Frontend, Backend and Dependency Audit jobs mirror what a developer can run locally; the Docker job does not.
...
### Purpose
Diagram 5 shows *which* status transitions are legal; this one shows the full engineering path a ticket-mutation request actually takes, and the guarantees around it — authorization, validation, optimistic concurrency, atomic history, and conditional SLA integration. It's drawn wide (left-to-right), not tall, specifically so it reads on GitHub without heavy vertical scrolling.

### Four endpoints, one pattern — not one endpoint
Ticket mutations are **not** all `PATCH /tickets/:id`. There are four distinct routes, each backed by its own service method: `PATCH /tickets/:id` (field edits — subject/description/category; `TicketsController.update`), `PATCH /tickets/:id/assignment` (`assign`, `@Roles(STAFF)`), `PATCH /tickets/:id/status` (`updateStatus`, no route-level role restriction), and `PATCH /tickets/:id/priority` (`updatePriority`, `@Roles(STAFF)`). The diagram shows all four converging on the same underlying engineering pattern rather than picking one and implying it's the only mutation path.

### Request-processing order — corrected
Every route passes through the same global stages **before** any service code runs, in this order: `JwtAuthGuard` (401 on failure) → `RolesGuard` (403 at the route level, **before validation or any DB read**, but only for the two routes that declare `@Roles(STAFF)` — `/assignment` and `/priority`; the other two have no route-level role restriction at all) → the global `ValidationPipe` (400 on a malformed body) — validation is a framework-level concern that always runs ahead of the controller/service, never after a database read. Only once all of that has passed does `TicketsService` do anything: `getVisibleTicketOrThrow` (404 if the ticket doesn't exist or isn't visible to this caller) → an endpoint-specific business-rule/ownership check → the CAS `updateMany` → the transaction's conditional side effects → commit → the post-commit audit write.

### The business-rule check is genuinely endpoint-specific
The diagram folds this into one decision box with a side note, because the real order and content differs by endpoint rather than following one canonical sequence:
- **Field edit** (`update`) — a non-staff requester is blocked (403) once the ticket's status has left `New`.
- **Status change** (`updateStatus` → `applyStatusTransition`) — checked in this exact order: a no-op ("already in that status") throws `400` first, then the transition-matrix check from diagram 5 throws `403` if the target status isn't reachable from the current one, then — only if neither of those fired — a staff-or-reopening check throws `403` for a non-staff caller attempting anything other than the one reopen transition.
- **Assignment / priority** (`assign`, `updatePriority`) — `RolesGuard` already restricts these routes to staff before validation ever runs (see above); the service *additionally* re-checks staff status as defense-in-depth, ahead of its own `getVisibleTicketOrThrow` call — a real ordering difference from the other two endpoints, but one that's unobservable from outside the process, since a non-staff caller is already rejected by the guard before reaching any of this.
...
- plain field edits — CAS on `updatedAt`
- assignment — CAS on `assigneeId`
- priority change — CAS on `priority`
- a status transition — CAS on `status`

If the predicate matches zero rows (`updateMany` count ≠ 1), someone else changed that exact field first — the request throws a `ConflictException` (`409`, "Ticket was modified by another request; reload and retry") rather than silently overwriting. A conditional SLA side effect follows **only** for operations that actually affect a clock (entering/leaving `OnHold`, resolution, reopen, or a priority change — a plain subject/description edit triggers none), then a `TicketHistory` row write, in the **same transaction and commit** as the ticket update itself. `AuditService.record(...)` is called **after** the transaction commits, deliberately **outside** it, specifically because `AuditService` never throws — a failed audit write can't fail or silently undo an already-committed ticket update.

### Failure branches, exactly as implemented
`401` unauthenticated · `403` a staff-only route blocked at the guard, **or** a failed ownership/business-rule check, **or** a transition the matrix disallows · `404` ticket not found or out of visibility scope · `400` malformed request body **or** a no-op status change · `409` CAS conflict. Underlying Prisma errors are mapped rather than left raw: `P2003` (foreign-key violation) → `400`, `P2025` (record not found) → `404`. There is no separate generic "transaction failure" branch beyond these.

### Simplifications
The business-rule/ownership check is drawn as one decision box rather than four separate per-endpoint diagrams — its real, endpoint-specific order is spelled out in the note above and on the diagram itself rather than implied by a single arrow. SLA-hook detail (exactly which of `OnHold` pause/resume, resolution, or reopen fires) is summarized rather than enumerated per branch — see diagram 8 for that detail.
